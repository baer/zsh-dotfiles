...
- Platform detection: `is_macos` and `is_linux` helpers (from `system/_platform.zsh`) are available in `*.zsh` files. In `path.zsh` files, use inline `[[ "$(uname -s)" == "Darwin" ]]` guards instead (path files load before the helpers).
